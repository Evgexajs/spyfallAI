...
### Что сделано
- Запустил `python3 -m src.analyze --game-id` на 3 разных логах партий:
  - `bbb6b856-1919-475e-aca9-234ac69c3b0c`
  - `bfd715e5-39be-4480-a143-a5e25827741c`
  - `80ad02a0-e3f0-44fd-a917-5344c95ce02a`
- Все 3 анализа завершились со статусом `completed`
- Проверил структуру `post_game_analysis` — соответствует CR-003 раздел 6:
  - `analyzed_at`, `analyzer_model`, `status`, `per_character` — все поля присутствуют
  - Каждый персонаж имеет `markers` и `must_compliance` с корректной структурой
  - `per_marker` содержит `marker_id`, `triggered_count`, `total_relevant_replies`, `rate`, `per_turn`
  - `per_directive` содержит `directive`, `satisfied`, `evidence_turns`, `reasoning`
- Проверил token_usage — стоимость каждой партии: $0.05-0.06 (включая игру + анализ)
  - Стоимость анализа оценивается ~$0.002-0.005 на партию — в пределах лимита $0.10
- Проверил graceful degradation — все персонажи во всех партиях проанализированы успешно
- Все 38 юнит-тестов PostGameAnalyzer проходят успешно
...
### Что сделано
- Добавлен автозапуск пост-анализа в `src/cli.py`:
  - Импорт `PostGameAnalyzer` и `POST_GAME_ANALYSIS_ENABLED` из `src/post_game`
  - Добавлен CLI флаг `--skip-analysis` для пропуска анализа
  - Анализ запускается ПОСЛЕ сохранения лога партии (`save_game()`)
  - Обёрнут в try/except — ошибка анализа не ломает сохранение партии
  - Если `POST_GAME_ANALYSIS_ENABLED=false` или `--skip-analysis` — анализ пропускается
  - Вывод статуса анализа в консоль (успех/ошибка)
...
## [TASK-088] CLI команда analyze для ретроспективного анализа
**Дата:** 2026-04-21
**Статус:** done

### Что сделано
- Создан `src/analyze.py` — CLI модуль для ретроспективного анализа партий:
  - `--game-id <uuid>` — анализ конкретной партии по ID
  - `--no-overwrite` — пропуск если анализ уже существует
  - `--all` — анализ всех логов без post_game_analysis
  - Возвращает exit code 0 при успехе, 1 при ошибках
  - Выводит статус и ошибки в stdout
- Добавлена функция `find_game_path_by_id(game_id)` для поиска файла по UUID
- Функции `has_post_game_analysis()` и `get_game_id_from_path()` для работы с файлами
- Команда запуска: `python -m src.analyze --game-id <uuid>`
...
## [TASK-087] Запись результата анализа в лог партии
**Дата:** 2026-04-21
**Статус:** done

### Что сделано
- Реализован метод `save_analysis(game_path, analysis)` в `src/post_game/analyzer.py`:
  - Читает существующий JSON-лог партии
  - Добавляет поле `post_game_analysis` с результатами анализа
  - Существующие поля лога сохраняются без изменений
  - Если `post_game_analysis` уже есть — перезаписывается с WARNING в лог
  - Использует `model_dump(mode="json")` для корректной сериализации datetime
  - Сохраняет JSON с `ensure_ascii=False, indent=2` для читаемости
...
### Что сделано
- Реализована функция `build_analysis_prompt(character, turns)` в `src/post_game/prompts.py`
- Промпт включает профиль персонажа: имя, архетип, стиль речи
- Промпт включает detectable_markers с id, description, method и check_question/rule
- Промпт включает must_directives персонажа
- Все реплики персонажа форматируются с контекстом (previous_speaker, previous_content, addressee)
- Явная инструкция возвращать JSON в формате CR-003 раздел 6.2
- Запрет markdown-обёртки и комментариев ("без ```json, без текста до/после")
- Вспомогательные функции: `_format_marker()`, `_format_turn()`

### Тестовые шаги (пройдены)
- Шаг 1: Собрал промпт для персонажа Зоя из characters/zoya.json ✓
- Шаг 2: Проверил наличие секций: профиль, маркеры, MUST-директивы, реплики, формат JSON ✓
...
## [TASK-082] Реализовать сбор реплик персонажа из лога партии
**Дата:** 2026-04-21
**Статус:** done

### Что сделано
- Реализован метод `_collect_character_turns(game, character_id)` в `src/post_game/analyzer.py`
- Метод фильтрует `game.turns` по `speaker_id == character_id`
- Возвращает `list[Turn]` в хронологическом порядке
- Каждый Turn содержит контекст: `turn_number` (для поиска предыдущей реплики), `addressee_id`, `type`
- Документация в docstring объясняет как получить предыдущий ход через `game.turns[turn.turn_number - 2]`

### Тестовые шаги (пройдены)
- Шаг 1: Загрузил тестовый лог партии из games/ ✓
- Шаг 2: Вызвал метод для персонажа "lyokha" — получено 7 ходов ✓
- Шаг 3: Проверил контекст — turn_number, addressee_id, type присутствуют, предыдущий ход доступен ✓
