...
МИРНЫЕ — твоя задача НАЙТИ шпиона:
• Задавай КОНКРЕТНЫЕ вопросы про детали локации: запахи, звуки, оборудование, одежду, действия
• Шпион не знает где вы — его ответы будут РАСПЛЫВЧАТЫМИ и ОБЩИМИ
• Если кто-то отвечает философией вместо конкретики — ЭТО ПОДОЗРИТЕЛЬНО
• Не бойся ОБВИНЯТЬ: «Ты подозрительно уклончив», «Ты явно не знаешь где мы»
• Используй слова: «подозрительно», «странно», «не верю», «ты шпион?»

ПРИЗНАКИ ШПИОНА (обвиняй если видишь):
• Отвечает абстракциями: «жизнь», «судьба», «важно», «интересно» — без деталей
• Переводит тему на философию, мораль, чувства
• Не называет конкретных вещей/действий которые есть на локации
• Задаёт слишком общие вопросы: «как тебе здесь?», «что думаешь?»
• Повторяет фразы других вместо своих конкретных ответов

ШПИОН — твоя задача ПОНЯТЬ локацию:
• Слушай КОНКРЕТНЫЕ детали: еда, оборудование, униформа, звуки
• Задавай вопросы которые звучат конкретно но не выдают незнание
• Если понял локацию — можешь рискнуть и угадать"""
...
ВАЖНО: Если ответ был РАСПЛЫВЧАТЫМ или АБСТРАКТНЫМ (философия, мораль, "жизнь", "интересно") —
это ПРИЗНАК ШПИОНА. Прямо укажи на это! Используй слова: "подозрительно", "уклончиво", "ты что-то скрываешь".

Напиши ОДНУ фразу вмешательства в своём стиле. Не более 1-2 предложений. Будь ПРЯМЫМ."""
...
def build_spy_confidence_check_prompt(
    ...
    """
    return f"""Ты — ШПИОН в игре Spyfall. Анализируй разговор и пойми где вы находитесь.
...
{{
  ...
  "location_guess": "твоя догадка или null",
  "confidence": "no_idea | few_guesses | confident",
  "reasoning": "почему такая уверенность"
}}

Уровни уверенности:
- "confident" — ты готов поставить деньги на эту локацию без сомнений. Требуется:
  • Минимум 4 РАЗНЫХ конкретных подсказки (не синонимы, а разные детали)
  • Подсказки ОДНОЗНАЧНО указывают на ОДНУ локацию, исключая все остальные
  • Если хоть одна другая локация подходит под подсказки — это НЕ confident
- "few_guesses" — есть догадка, но недостаточно уверенности для ставки
- "no_idea" — подсказок мало или они противоречивы

ПРИМЕРЫ когда НЕ confident:
- "Музыка + ароматы" — подходит и спа, и отель, и ресторан → few_guesses
- "Шум + теснота" — подходит метро, самолёт, подлодка, завод → few_guesses
- "Вода + расслабление" — спа, бассейн, пляж → few_guesses

ПРИМЕР confident: "красный свет + механический гул + ограниченное пространство + упоминание глубины + давление" — только подводная лодка"""
