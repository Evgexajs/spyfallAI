...
while has_pending_tasks; do
    ...
    prompt=$(cat <<EOF
@${TASKS_FILE} @progress.md @../docs/PRD-Visualizer-v0.3.md
...
## Контекст проекта:
Visualizer — визуализатор диалоговых партий SpyfallAI на PixiJS.
Стек: TypeScript, PixiJS, Vite, HTML/CSS.
Структура: src/ (parser, player, render, ui, config), assets/ (locations, fonts).

## Архитектура (4 изолированных слоя):
- parser: валидация JSON → GameData
- player: state machine воспроизведения, таймеры
...
- События timeline строго последовательно
- CharacterRenderer — интерфейс для персонажей (замена реализации без изменений в других слоях)
- Визуализатор — единственный источник правды по таймингам
...
EOF
    ...
    if [[ "$result" == *"<promise>COMPLETE</promise>"* ]]; then
        ...
        if [[ "$remaining" -eq 0 ]]; then
            ...
            say -v Milena "[[volm 0.3]] Хозяин, визуализатор готов!"
            exit 0
        fi
